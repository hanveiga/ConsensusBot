...
def generate_dialog():
    ...
    dialog.append({'user_e':'John','created_at':datetime.now(),'message_e':'I can do from 6 on Sunday'})
    dialog.append({'user_e':'Mark','created_at':datetime.now(),'message_e':'I can do at 9 on Sunday'})
...
def simulate_bot_session(meeting_suggestion):
    message_stack = generate_dialog()
